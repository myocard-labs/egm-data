...
def _write_banks_group(f: h5py.File, banks: list[ClassifierBankMetaData]) -> None:
    ...
    b = len(banks)
    g.create_dataset("bank_id", data=np.asarray([m.bank_id for m in banks], dtype=np.int64))
    g.create_dataset(
        "bank_type",
...
def _write_traces_group(f: h5py.File, traces: list[ClassifierTrace], T: int) -> None:
    ...
    g.create_dataset("bank_id", data=np.asarray([t.bank_id for t in traces], dtype=np.int64))
...
def _read_banks_group(g: h5py.Group) -> list[ClassifierBankMetaData]:
    bank_ids = np.asarray(g["bank_id"][...], dtype=np.int64)
...
def _read_traces_group(g: h5py.Group) -> list[ClassifierTrace]:
    ...
    bank_id = np.asarray(g["bank_id"][...], dtype=np.int64)
